...
class PhyloProcess : public virtual SubstitutionProcess, public virtual BranchProcessVI {
	...
	*/
	...
	void SampleSubstitutionMappings(const Link* from);
        double NumberTypeSubstitution(const Link* from);
        double numbertype;
        double GetNumberTypeStatesVI(int* stateup, int* statedown, double time); 
        double typeaminoacid;       
	...
	*/
	...
	// int GetSiteRateSuffStatCount(int site) {return siteratesuffstatcount[site];}
        int GetSiteRateSuffStatCount(int site)     {
            GetTotalSitePCAT(); 
            return Getproclus() * siteratesuffstatcount[site];
        }
	...
	*/
	...
	// int GetBranchLengthSuffStatCount(int index) {return branchlengthsuffstatcount[index];}
        int GetBranchLengthSuffStatCount(int index)   {
            GetTotalSitePCAT();
            return Getproclus() * branchlengthsuffstatcount[index];
        }
