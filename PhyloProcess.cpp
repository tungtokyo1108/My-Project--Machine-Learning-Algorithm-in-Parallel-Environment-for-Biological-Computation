...
void PhyloProcess::Collapse()	{
	...
	SampleSubstitutionMappings(GetRoot());
        NumberTypeSubstitution(GetRoot());
	CreateSuffStat();
}
...
double PhyloProcess::ComputeNodeLikelihood(const Link* from, int auxindex) {
       ...
       for (const Link* link=from->Next(); link!=from; link=link->Next())  {
              if (! link->isRoot())  {
                    // MultiplyByStationaries(GetConditionalLikelihoodVector(link));
                    lnL = ComputeLikelihood(GetConditionalLikelihoodVector(link));  
              }
       }       
...
void PhyloProcess::SampleNodeStates(const Link* from, double** aux)  {
     ...
     for (const Link* link=from->Next(); link!=from; link=link->Next())  {
             // ChooseStates(GetConditionalLikelihoodVector(link->Out()),GetStates(link->GetNode()));
             Propagate(aux,GetConditionalLikelihoodVector(link->Out()),GetLength(link->GetBranch()),true);
     }
     for (const Link* link=from->Next(); link!=from; link=link->Next())  {
             SampleNodeStates(link->Out(),aux);
...
double PhyloProcess::NumberTypeSubstitution(const Link* from)     {
       ...
       numbertype = GetNumberTypeStatesVI(GetStates(from->Out()->GetNode()), GetStates(from->GetNode()), GetLength(from->GetBranch()));
       /*for (const Link* link=from->Next(); link!=from; link=link->Next())   {
              NumberTypeSubstitution(link->Out());
       }*/
       return numbertype;
}

/*double PhyloProcess::GetNumberTypeStatesVI(int* stateup, int* statedown, double time)     {
       ...
       for (int i=sitemin; i<sitemax; i++)    {
               ...
               if (dup == ddown)     {
                      ...
                      norm += w;
                      total += w * (1-exp(-l)) * pidown / (exp(-l) + (1-exp(-l)) * pidown);
               }
               else   {
                      total = 1;
               }
       ...
       return typeaminoacid;
}*/

/*double PhyloProcess::GetNumberTypeStatesVI(int* stateup, int* statedown, double time)   {

        for (int i=sitemin; i<sitemax; i++)    {
             double* pi = GetStationaryDirweightVI(i);
...
double PhyloProcess::LocalBranchLengthMove(const Link* from, double tuning)  {
       ...
       for (const Link* link=from->Next(); link!=from; link=link->Next())  {
              GlobalPropagate(0,link,GetLength(link->GetBranch()));
       }
       ...
       if (!accepted)  {
               for (const Link* link=from->Next(); link!=from; link=link->Next())  {
                       GlobalPropagate(0,link,GetLength(link->GetBranch()));
                       GlobalComputeNodeLikelihood(link);
               }
       }
       return (double) accepted;
}
...
void PhyloProcess::Create(Tree* intree, SequenceAlignment* indata,int indim)	{
	...
	if (! data)	{
		...
		RateProcessVI::Create(data->GetNsite());
